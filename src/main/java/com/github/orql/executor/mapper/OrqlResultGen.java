...
import com.github.orql.core.Constants;
    ...
    private ResultRoot toResult(OrqlRefItem orqlRoot, String path, List<ResultColumn> allColumns) {
        ...
        for (OrqlItem item : orqlRoot.getChildren()) {
            if (item instanceof OrqlAllItem) {
                ...
                ColumnInfo column = ((OrqlColumnItem) item).getColumn();
        ...
        if (selectAll) {
            for (ColumnInfo column : orqlRoot.getRef().getColumns()) {
        ...
        // 没有id插入id
        if (resultId == null) {
            SchemaInfo refSchema = orqlRoot.getRef();
